...
namespace Application
{
	...
	void MainFrameWindow::initialise()
	{
		...
		Bind( wxEVT_COMMAND_MENU_SELECTED,
			  ...
			  ID_ABOUT);
		Bind( wxEVT_COMMAND_MENU_SELECTED,
			  [this](CommandEvent& anEvent){ this->OnKalman(anEvent);},
			  ID_KALMAN);
		Bind( wxEVT_COMMAND_MENU_SELECTED,
			  [this](CommandEvent& anEvent){ this->OnParticle(anEvent);},
			  ID_PARTICLE);
	...
	MenuBar* MainFrameWindow::initialiseMenuBar()
	{
		...
		filterMenu->Append( ID_KALMAN, WXSTRING( "&Kalman\tF2"), WXSTRING( "Drive based on the Kalman Filter"));
		filterMenu->Append( ID_PARTICLE, WXSTRING( "&Particle\tF3"), WXSTRING( "Drive based on the Particle Filter"));

		menuBar = new wxMenuBar;
		menuBar->Append( fileMenu, WXSTRING( "&File"));
		menuBar->Append( debugMenu, WXSTRING( "&Debug"));
		menuBar->Append( helpMenu, WXSTRING( "&Help"));
		menuBar->Append( filterMenu, WXSTRING( "&Filter"));
	...
	Panel* MainFrameWindow::initialiseRhsPanel()
	{
		if (!rhsPanel)
		{
			...
			sizer->AddGrowableCol( 1);
			sizer->AddGrowableRow( 1);
			logTextCtrl->SetMinSize( Size( 1024, 300));
	...
	{
		wxMessageBox( WXSTRING( "Enabled Kalman Filter.\n"), WXSTRING( "RobotWorld Options"), wxOK | wxICON_INFORMATION, this);
		if(!Model::Robot::KalmanActivated)
		{
			Model::Robot::KalmanActivated = true;
			Model::Robot::ParticleActivated = false;
		}
	}
	/**
	 *
	...
	{
		wxMessageBox( WXSTRING( "Enabled Particle Filter.\n"), WXSTRING( "RobotWorld Options"), wxOK | wxICON_INFORMATION, this);
		if(!Model::Robot::ParticleActivated)
		{
			Model::Robot::ParticleActivated = true;
			Model::Robot::KalmanActivated = false;
		}
	}
	/**
	 *
